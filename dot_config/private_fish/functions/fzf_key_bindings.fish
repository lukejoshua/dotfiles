...
function fzf_key_bindings
  ...
  function fzf-home-path-widget -d "Insert path from home directory"
    set -l home_dir $HOME
    set -l fzf_query (commandline -t)

    set -lx FZF_DEFAULT_OPTS (__fzf_defaults \
      "--reverse --walker=dir,follow,hidden --scheme=path --walker-root=$home_dir" \
      "--no-multi")
  ...
  bind \cp fzf-home-path-widget
  bind -M insert \cp fzf-home-path-widget
